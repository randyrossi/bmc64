...
int up_load_manifests(uz_zip *z, um_manifest *m, char *err, unsigned errlen) {
  ...
  if (index < 0) {
    snprintf(err, errlen,
             "It has no " UM_NAME ". Only releases made after the updater "
             "was added can be installed this way.");
    return -1;
  }
  struct grow g = {0};
  ...
  if (f) {
    ...
    unsigned got = 0;
    if (text && uf_read(f, text, size, &got) == 0 && got == size) {
      text[size] = '\0';
      // Only adds history; a damaged copy just means less of it.
      char ignore[64];
      um_parse(m, text, 0, ignore, sizeof(ignore));
    } else {
      free(text);
    }
    uf_close(f);
