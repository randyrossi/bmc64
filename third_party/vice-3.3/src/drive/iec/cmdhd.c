...
static uint8_t drive_read_rom(drive_t *drv, uint16_t address)
{
    return drv->rom[address & 0x7fff];
}

static uint8_t drive_read_ram(drive_t *drv, uint16_t address)
{
    return drv->drive_ram[address];
}

static void drive_store_ram(drive_t *drv, uint16_t address, uint8_t value)
{
...
void cmdbus_update(void)
{
    ...
    for (i = 0; i < DRIVE_NUM; i++) {
        ...
        if ((cmdbus.drv_bus[i] & 1) && (drive_context[i]->drive->parallel_cable)) {
...
static int cmdhd_patn_changed(unsigned int unit, int new, int old)
{
    ...
    /* check context */
    if (!drive_context[unit - 8]) {
        return -1;
    }

    if (drive_context[unit - 8]->drive->type != DRIVE_TYPE_CMDHD) {
        return -1;
    }

    /* get context */
    hd = drive_context[unit - 8]->cmdhd;
...
void cmdbus_patn_changed(int new, int old)
{
    ...
    for (unit = 8; unit < 8 + DRIVE_NUM; unit++ ) {
...
static void updateleds(drive_t *ctxptr)
{
    ctxptr->led_status = (drive_context[ctxptr->mynumber]->cmdhd->LEDs & 0x02) ? 1 : 0;
    ctxptr->led_status |= (drive_context[ctxptr->mynumber]->cmdhd->LEDs & 0x01) ? 2 : 0;
}
...
#define storedebug()
    ...
    case 0x7:
        ...
        if (drive_context[ctxptr->mynumber]->cmdhd->i8255a_o[2]&2) {
            drive_store_ram(ctxptr->drive, (addr & 0x3fff) | 0x4000, data );
        } else {
            drive_store_ram(ctxptr->drive, (addr & 0x3fff) | 0xC000, data );
        }
        break;
    case 0x8:
        /* Decode bits 11-8 */
        ...
        case 0x1: /* 0x81xx U10 */
            storedebug()
            viacore_store(drive_context[ctxptr->mynumber]->cmdhd->via10, addr & 15, data);
            break;
        case 0x4: /* 0x84xx U9 */
        case 0x5: /* 0x85xx U9 */
            storedebug()
            viacore_store(drive_context[ctxptr->mynumber]->cmdhd->via9, addr & 15, data);
            break;
        case 0x8: /* 0x88xx U11 */
        case 0x9: /* 0x89xx U11 */
            storedebug()
            i8255a_store(drive_context[ctxptr->mynumber]->cmdhd->i8255a, addr & 3, data);
            break;
        case 0xc: /* 0x8cxx */
        case 0xd: /* 0x8dxx */
            storedebug()
            rtc72421_write(drive_context[ctxptr->mynumber]->cmdhd->rtc, addr & 15, data);
            break;
        case 0xf: /* 0x8fxx U20 */
            /* Although page 0x8F is RAM, all writes go to U20 */
            /* OS often reads from 0x8f00 to get past values to OR/AND them */
            storedebug()
            drive_context[ctxptr->mynumber]->cmdhd->LEDs = data;
            drive_store_ram(ctxptr->drive, (addr & 255) | 0x8f00, data);
            updateleds(ctxptr->drive);
            break;
        case 0xe: /* 0x8exx unprotected RAM */
            drive_store_ram(ctxptr->drive, (addr & 255) | 0x8e00, data);
            break;
        default:
            /* Everything else writes to RAM if switch is on */
            if (drive_context[ctxptr->mynumber]->cmdhd->LEDs&32) {
                drive_store_ram(ctxptr->drive, addr, data);
            }
            break;
        }
        break;
...
#define readdebug()
    ...
    case 0x7:
        /* Since the ROM wants to read/write from the RAM from 0xC000-0xFFFF,
            when 0x8802.1 = 0, RAM from 0xC000-0xFFFF maps to 0x4000-0x7FFF */
        if (drive_context[ctxptr->mynumber]->cmdhd->i8255a_o[2]&2) {
            return drive_read_ram(ctxptr->drive, (addr & 0x3fff) | 0x4000 );
        } else {
            return drive_read_ram(ctxptr->drive, (addr & 0x3fff) | 0xC000 );
        }
        break;
    case 0x8:
        /* Decode bits 11-8 */
        ...
        case 0x0: /* 0x80xx U10 */
        case 0x1: /* 0x81xx U10 */
            data = viacore_read(drive_context[ctxptr->mynumber]->cmdhd->via10, addr & 15);
            readdebug()
            return data;
            break;
        case 0x4: /* 0x84xx U9 */
        case 0x5: /* 0x85xx U9 */
            data = viacore_read(drive_context[ctxptr->mynumber]->cmdhd->via9, addr & 15);
            readdebug()
            return data;
            break;
        case 0x8: /* 0x88xx U11 */
        case 0x9: /* 0x89xx U11 */
            data = i8255a_read(drive_context[ctxptr->mynumber]->cmdhd->i8255a, addr & 3);
            readdebug()
            return data;
            break;
        case 0xc: /* 0x8cxx */
        case 0xd: /* 0x8dxx */
            data = rtc72421_read(drive_context[ctxptr->mynumber]->cmdhd->rtc, addr & 15);
            readdebug()
            return data;
            break;
        default:
            return drive_read_ram(ctxptr->drive, addr);
            break;
        }
        break;
    case 0x9:
    ...
    case 0xf:
        ...
        if (drive_context[ctxptr->mynumber]->cmdhd->i8255a_o[2]&1) {
            return drive_read_rom(ctxptr->drive, addr & 0x3fff );
        } else {
            return drive_read_ram(ctxptr->drive, addr);
        }
        break;
...
uint8_t cmdhd_peek(struct drive_context_s *ctxptr, uint16_t addr)
{
    return 0;
}

int cmdhd_dump(struct drive_context_s *ctxptr, uint16_t addr)
{
    return 0;
}
                    ...
                    int value, CLOCK rclk)
{
    ...
    drive_t *dc = (drive_t *)(hd->mycontext);

    interrupt_set_irq(drive_context[dc->mynumber]->cpu->int_status, int_num, value, rclk);
}
                        ...
                        int value)
{
    ...
    interrupt_restore_irq(drive_context[dc->mynumber]->cpu->int_status, int_num, value);
}
...
void cmdhd_setup_context(struct drive_context_s *ctxptr)
{
    ...
    drive_context[ctxptr->mynumber]->cmdhd = lib_calloc(1, sizeof(cmdhd_context_t));
    drive_context[ctxptr->mynumber]->cmdhd->myname = lib_msprintf("CMDHD%d", ctxptr->mynumber);
    drive_context[ctxptr->mynumber]->cmdhd->mycontext = ctxptr->drive;

    drive_context[ctxptr->mynumber]->cmdhd->image = NULL;

    /* Clear struct as snapshot code may write uninitialized values.  */
    drive_context[ctxptr->mynumber]->cmdhd->via10 = lib_calloc(1, sizeof(via_context_t));
    via10 = drive_context[ctxptr->mynumber]->cmdhd->via10;

    via10->prv = lib_malloc(sizeof(drivevia_context_t));
    via10p = (drivevia_context_t *)(via10->prv);
    via10p->number = ctxptr->mynumber;

    via10->context = (void *)drive_context[ctxptr->mynumber]->cmdhd;

    via10->rmw_flag = &(drive_context[ctxptr->mynumber]->cpu->rmw_flag);
    via10->clk_ptr = drive_context[ctxptr->mynumber]->clk_ptr;
    ...
    drive_context[ctxptr->mynumber]->cmdhd->via9 = lib_calloc(1, sizeof(via_context_t));
    via9 = drive_context[ctxptr->mynumber]->cmdhd->via9;

    via9->context = (void *)drive_context[ctxptr->mynumber]->cmdhd;

    via9->rmw_flag = &(drive_context[ctxptr->mynumber]->cpu->rmw_flag);
    via9->clk_ptr = drive_context[ctxptr->mynumber]->clk_ptr;
    ...
    drive_context[ctxptr->mynumber]->cmdhd->scsi = lib_calloc(1, sizeof(scsi_context_t));
    scsi = drive_context[ctxptr->mynumber]->cmdhd->scsi;
    scsi->p = drive_context[ctxptr->mynumber]->cmdhd;
    scsi->myname = lib_msprintf("CMDHD%dSCSI", ctxptr->mynumber);

    drive_context[ctxptr->mynumber]->cmdhd->i8255a = lib_calloc(1, sizeof(i8255a_state));
    i8255a = drive_context[ctxptr->mynumber]->cmdhd->i8255a;
    i8255a->p = drive_context[ctxptr->mynumber]->cmdhd;
    ...
    drive_context[ctxptr->mynumber]->cmdhd->rtc = rtc72421_init(name);
    lib_free(name);

    drive_context[ctxptr->mynumber]->cmdhd->rtc->stop = 0;

    name = lib_msprintf("%sEXEC", drive_context[ctxptr->mynumber]->cmdhd->myname);
    drive_context[ctxptr->mynumber]->cmdhd->reset_alarm = alarm_new(drive_context[ctxptr->mynumber]->cpu->alarm_context, name,
        reset_alarm_handler, drive_context[ctxptr->mynumber]->cmdhd);
    lib_free(name);

    /* reset attachment counter for warning messages */
    drive_context[ctxptr->mynumber]->cmdhd->numattached = 0;
}

void cmdhd_init(struct drive_context_s *ctxptr)
{
    scsi_context_t *scsi = (scsi_context_t*)(drive_context[ctxptr->mynumber]->cmdhd->scsi);

    CLOG((LOG, "CMDHD: init"));

    /* init via cores */
    viacore_init(drive_context[ctxptr->mynumber]->cmdhd->via9, drive_context[ctxptr->mynumber]->cpu->alarm_context,
                 drive_context[ctxptr->mynumber]->cpu->int_status, drive_context[ctxptr->mynumber]->cpu->clk_guard);
    viacore_init(drive_context[ctxptr->mynumber]->cmdhd->via10, drive_context[ctxptr->mynumber]->cpu->alarm_context,
                 drive_context[ctxptr->mynumber]->cpu->int_status, drive_context[ctxptr->mynumber]->cpu->clk_guard);
    ...
    scsi->max_imagesize = ctxptr->drive->fixed_size;
    ...
    drive_context[ctxptr->mynumber]->cmdhd->preadyff = 1;
}
...
int cmdhd_attach_image(disk_image_t *image, unsigned int unit)
{
    ...
    switch (image->type) {
        case DISK_IMAGE_TYPE_DHD:
            disk_image_attach_log(image, LOG, unit);
            break;
        default:
            return -1;
    }
    ...
    /* look to see if there are more files with the same base
       ...
       s<ID><LUN> */
    /* copy the file name */
    basename = strdup(image->media.fsimage->name);
...
int cmdhd_detach_image(disk_image_t *image, unsigned int unit)
{
    ...
    switch (image->type) {
        case DISK_IMAGE_TYPE_DHD:
            disk_image_detach_log(image, LOG, unit);
            break;
        default:
            return -1;
    }
...
int cmdhd_snapshot_read_module(cmdhd_context_t *drv, struct snapshot_s *s)
{
    ...
    if (snapshot_version_at_least(vmajor, vminor, CMDHD_SNAP_MAJOR, CMDHD_SNAP_MAJOR)) {
