...
CViceScreenDevice::CViceScreenDevice (unsigned nWidth, unsigned nHeight, boolean bVirtual)
:	m_nInitWidth (nWidth),
...
boolean CViceScreenDevice::Initialize (void)
{
	if (!m_bVirtual)
	{
		...
		m_pFrameBuffer = new CBcmFrameBuffer (m_nInitWidth, m_nInitHeight, DEPTH, m_nInitWidth, m_nInitHeight * 2);
#if DEPTH == 8
...
unsigned CViceScreenDevice::GetWidth (void) const
{
	return m_nWidth;
}

unsigned CViceScreenDevice::GetHeight (void) const
{
	return m_nHeight;
}
...
void CViceScreenDevice::CarriageReturn (void)
{
	m_nCursorX = 0;
}
...
void CViceScreenDevice::InsertMode (boolean bBegin)
{
	m_bInsertOn = bBegin;
}
...
void CViceScreenDevice::DisplayChar (char chChar, unsigned nPosX, unsigned nPosY, TScreenColor Color)
{
...
void CViceScreenDevice::SetPixel (unsigned nPosX, unsigned nPosY, TScreenColor Color)
{
	if (   nPosX < m_nWidth
	    && nPosY < m_nHeight)
	{
...
void CViceScreenDevice::UpdatePalette() {
